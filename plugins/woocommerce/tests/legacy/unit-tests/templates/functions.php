...
class WC_Tests_Template_Functions extends WC_Unit_Test_Case {
	...
	public function test_wc_query_string_form_fields() {
		$actual_html   = wc_query_string_form_fields( '?test=1', [], '', true );
		$expected_html = '<input type="hidden" name="test" value="1" />';
		$this->assertEquals( $expected_html, $actual_html );

		$actual_html   = wc_query_string_form_fields( '?test=1&test2=something', [], '', true );
		$expected_html = '<input type="hidden" name="test" value="1" /><input type="hidden" name="test2" value="something" />';
		$this->assertEquals( $expected_html, $actual_html );

		$actual_html   = wc_query_string_form_fields( '?test.something=something.else', [], '', true );
		...
		$actual_html   = wc_query_string_form_fields( '?test%20something=something%20else', [], '', true );
	...
	public function test_hidden_field() {
		...
		[
			'type' => 'hidden',
			'id' => 'test_field',
			'input_class' => [ 'test-field' ],
			'custom_attributes' => [ 'data-total' => '10' ],
			'return' => true
		], 'test value');
