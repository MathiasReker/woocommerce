...
class WC_Tests_Tax extends WC_Unit_Test_Case {
	...
	public function test_calc_tax() {
		...
		$this->assertEquals( $calced_tax, [ $tax_rate_id => '1.665' ] );

		$calced_tax = WC_Tax::calc_tax( '9.99', $tax_rates, false, false );

		$this->assertEquals( $calced_tax, [ $tax_rate_id => '1.998' ] );
	}
	...
	public function test_calc_shipping_tax() {
		...
		$this->assertEquals( $calced_tax, [ $tax_rate_id => '2' ] );
	}
	...
	public function test_get_tax_classes() {
		...
		$this->assertEquals( $tax_classes, [ 'Reduced rate', 'Zero rate' ] );

		$tax_class_slugs = WC_Tax::get_tax_class_slugs();
		$this->assertEquals( $tax_class_slugs, [ 'reduced-rate', 'zero-rate' ] );
	...
	public function test__update_tax_rate_postcodes() {
		...
		$this->assertEquals( [ '12345', '90210...90215' ], $results );
	}
	...
	public function test__update_tax_rate_cities() {
		...
		$this->assertEquals( [ 'SOMEWHERE', 'SOMEWHERE_ELSE' ], $results );
	}
}
