...
class WC_Tests_Admin_Post_Types extends WC_Unit_Test_Case {
	...
	public function data_provider_bulk_and_quick_edit_stock_status_for_variable_product() {
		return [
			...
			[ 'quick_edit', '', 'outofstock' ],
			[ 'quick_edit', 'instock', 'instock' ],
			[ 'bulk_edit', '', 'outofstock' ],
			[ 'bulk_edit', 'instock', 'instock' ],
		];
	}
	...
	public function data_provider_bulk_change_price() {
		...
		foreach ( [ 'regular', 'sale' ] as $type ) {
			array_push( $dataset, [ $type, '10.33', '1', '5.339', 5.34 ] );
			array_push( $dataset, [ $type, '10.33', '2', '5.3333', 15.66 ] );
			array_push( $dataset, [ $type, '10.33', '2', '15.555%', 11.94 ] );
			array_push( $dataset, [ $type, '10.33', '3', '5.339', 4.99 ] );
			array_push( $dataset, [ $type, '10.33', '3', '15.555%', 8.72 ] );
		}

		array_push( $dataset, [ 'regular', '10.33', '4', '5.339', 10.33 ] );
		array_push( $dataset, [ 'regular', '10.33', '4', '15.555%', 10.33 ] );

		array_push( $dataset, [ 'sale', '10.33', '4', '5.339', 97.96 ] );
		array_push( $dataset, [ 'sale', '10.33', '4', '15.555%', 87.23 ] );
	...
	public function bulk_change_price( $type_of_price, $initial_price, $type_of_change, $change_amount, $expected_new_price ) {
		if ( 'regular' === $type_of_price ) {
			$props = [ 'regular_price' => $initial_price ];
		} else {
			$props = [
