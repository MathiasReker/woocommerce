	...
	public function generate_response( $data ) {
		if ( isset( $_GET['_jsonp'] ) ) {
			...
			if ( ! apply_filters( 'woocommerce_api_jsonp_enabled', true ) ) {
				...
				return wp_json_encode( [ [ 'code' => 'woocommerce_api_jsonp_disabled', 'message' => __( 'JSONP support is disabled on this site', 'woocommerce' ) ] ] );
			}
			...
			if ( ! wp_check_jsonp_callback( $jsonp_callback ) ) {
				WC()->api->server->send_status( 400 );
				return wp_json_encode( [ [ 'code' => 'woocommerce_api_jsonp_callback_invalid', __( 'The JSONP callback function is invalid', 'woocommerce' ) ] ] );
			}
