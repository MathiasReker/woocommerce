...
class WC_API_Taxes extends WC_API_Resource {
	...
	public function register_routes( $routes ) {
		...
		# GET/POST /taxes
		$routes[ $this->base ] = [
			[ [ $this, 'get_taxes' ], WC_API_Server::READABLE ],
			[ [ $this, 'create_tax' ], WC_API_Server::CREATABLE | WC_API_Server::ACCEPT_DATA ],
		];

		# GET /taxes/count
		$routes[ $this->base . '/count' ] = [
			[ [ $this, 'get_taxes_count' ], WC_API_Server::READABLE ],
		];

		# GET/PUT/DELETE /taxes/<id>
		$routes[ $this->base . '/(?P<id>\d+)' ] = [
			[ [ $this, 'get_tax' ], WC_API_Server::READABLE ],
			[ [ $this, 'edit_tax' ], WC_API_SERVER::EDITABLE | WC_API_SERVER::ACCEPT_DATA ],
			[ [ $this, 'delete_tax' ], WC_API_SERVER::DELETABLE ],
		];

		# GET/POST /taxes/classes
		$routes[ $this->base . '/classes' ] = [
			[ [ $this, 'get_tax_classes' ], WC_API_Server::READABLE ],
			[ [ $this, 'create_tax_class' ], WC_API_Server::CREATABLE | WC_API_Server::ACCEPT_DATA ],
		];

		# GET /taxes/classes/count
		$routes[ $this->base . '/classes/count' ] = [
			[ [ $this, 'get_tax_classes_count' ], WC_API_Server::READABLE ],
		];

		# GET /taxes/classes/<slug>
		$routes[ $this->base . '/classes/(?P<slug>\w[\w\s\-]*)' ] = [
			[ [ $this, 'delete_tax_class' ], WC_API_SERVER::DELETABLE ],
		];
	 ...
	 * @return array
	 */
	public function get_taxes( $fields = null, $filter = [], $class = null, $page = 1 ) {
		if ( ! empty( $class ) ) {
	...
	public function get_tax( $id, $fields = null ) {
		...
		try {
			...
			return [ 'tax' => apply_filters( 'woocommerce_api_tax_response', $tax_data, $tax, $fields, $this ) ];
		...
		try {
			...
			foreach ( $tax_data as $key => $value ) {
				...
				if ( isset( $data[ $new_key ] ) ) {
					if ( in_array( $new_key, [ 'compound', 'shipping' ] ) ) {
	...
	public function edit_tax( $id, $data ) {
		try {
			...
			foreach ( $data as $key => $value ) {
				...
				if ( in_array( $key, [ 'compound', 'shipping' ] ) ) {
					$value = $value ? 1 : 0;
				}
	...
	public function delete_tax( $id ) {
		...
		try {
			...
			return [ 'message' => sprintf( __( 'Deleted %s', 'woocommerce' ), 'tax' ) ];
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), [ 'status' => $e->getCode() ] );
		}
	}
	 ...
	 * @return array|WP_Error
	 */
	public function get_taxes_count( $class = null, $filter = [] ) {
		try {
			if ( ! current_user_can( 'manage_woocommerce' ) ) {
			...
			return [ 'count' => (int) $query['headers']->total ];
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), [ 'status' => $e->getCode() ] );
		}
	}
		...
		try {
			...
			foreach ( $data as $_tax ) {
				...
				if ( $tax_id ) {
					...
					$edit = $this->edit_tax( $tax_id, [ 'tax' => $_tax ] );
			...
			return [ 'taxes' => apply_filters( 'woocommerce_api_taxes_bulk_response', $taxes, $this ) ];
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), [ 'status' => $e->getCode() ] );
		}
	}
	...
	public function get_tax_classes( $fields = null ) {
		try {
			...
			foreach ( $classes as $class ) {
				$tax_classes[] = apply_filters( 'woocommerce_api_tax_class_response', [
					'slug' => sanitize_title( $class ),
					'name' => $class,
				], $class, $fields, $this );
			}

			return [ 'tax_classes' => apply_filters( 'woocommerce_api_tax_classes_response', $tax_classes, $classes, $fields, $this ) ];
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), [ 'status' => $e->getCode() ] );
		}
	}
	...
	public function delete_tax_class( $slug ) {
		...
		try {
			...
			return [ 'message' => sprintf( __( 'Deleted %s', 'woocommerce' ), 'tax_class' ) ];
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), [ 'status' => $e->getCode() ] );
		}
	}
