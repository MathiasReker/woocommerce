...
class WC_API_Customers extends WC_API_Resource {
	...
	public function register_routes( $routes ) {
		...
		$routes[ $this->base . '/email/(?P<email>.+)' ] = [
		...
		$routes[ $this->base . '/(?P<id>\d+)/downloads' ] = [
	...
	protected function get_customer_billing_address() {
		$billing_address = apply_filters( 'woocommerce_api_customer_billing_address', [
			'first_name',
			'last_name',
			'company',
			'address_1',
	...
	protected function get_customer_shipping_address() {
		$shipping_address = apply_filters( 'woocommerce_api_customer_shipping_address', [
			'first_name',
			'last_name',
			'company',
			'address_1',
		...
		if ( isset( $data['billing_address'] ) ) {
			foreach ( $this->get_customer_billing_address() as $field ) {
				if ( isset( $data['billing_address'][ $field ] ) ) {
					if ( is_callable( [ $customer, "set_billing_{$field}" ] ) ) {
		...
		if ( isset( $data['shipping_address'] ) ) {
			foreach ( $this->get_customer_shipping_address() as $field ) {
				if ( isset( $data['shipping_address'][ $field ] ) ) {
					if ( is_callable( [ $customer, "set_shipping_{$field}" ] ) ) {
	 ...
	 * @return array|WP_Error
	 */
	public function get_customer_orders( $id, $fields = null, $filter = [] ) {
		$id = $this->validate_request( $id, 'customer', 'read' );
	...
	public function get_customer_downloads( $id, $fields = null ) {
		...
		return [ 'downloads' => apply_filters( 'woocommerce_api_customer_downloads_response', $downloads, $id, $fields, $this->server ) ];
	}
		...
		try {
			...
			foreach ( $data as $_customer ) {
				...
				if ( $customer_id ) {
					...
					$edit = $this->edit_customer( $customer_id, [ 'customer' => $_customer ] );
					...
					$new = $this->create_customer( [ 'customer' => $_customer ] );
			...
			return [ 'customers' => apply_filters( 'woocommerce_api_customers_bulk_response', $customers, $this ) ];
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), [ 'status' => $e->getCode() ] );
		}
	}
