...
class WC_Settings_Payment_Gateways extends WC_Settings_Page {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_admin_field_payment_gateways_banner', [ $this, 'payment_gateways_banner' ] );
		add_action( 'woocommerce_admin_field_payment_gateways', [ $this, 'payment_gateways_setting' ] );
		parent::__construct();
	}
	...
	protected function get_settings_for_default_section() {
		$settings =
			[
				...
				[ 'type' => 'payment_gateways_banner' ], // React mount point for embedded banner slotfill.
				[
	...
	public function output() {
		...
		if ( $current_section ) {
			foreach ( $payment_gateways as $gateway ) {
				if ( in_array( $current_section, [ $gateway->id, sanitize_title( get_class( $gateway ) ) ], true ) ) {
					...
					<tbody>
						...
						foreach ( WC()->payment_gateways->payment_gateways() as $gateway ) {
							...
							foreach ( $columns as $key => $column ) {
								...
								if ( in_array( $key, [ 'sort', 'status', 'action' ], true ) ) {
									$width = '1%';
								}
