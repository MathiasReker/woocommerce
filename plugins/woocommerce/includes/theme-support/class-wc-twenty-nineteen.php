...
class WC_Twenty_Nineteen {
	...
	public static function init() {
		...
		add_action( 'woocommerce_before_main_content', [ __CLASS__, 'output_content_wrapper' ], 10 );
		add_action( 'woocommerce_after_main_content', [ __CLASS__, 'output_content_wrapper_end' ], 10 );
		...
		add_action( 'wp', [ __CLASS__, 'tweak_theme_features' ] );

		// Color scheme CSS.
		add_filter( 'twentynineteen_custom_colors_css', [ __CLASS__, 'custom_colors_css' ], 10, 3 );
	}
	...
	public static function enqueue_styles( $styles ) {
		...
		$styles['woocommerce-general'] = [
			'src'     => str_replace( [ 'http:', 'https:' ], '', WC()->plugin_url() ) . '/assets/css/twenty-nineteen.css',
			'deps'    => '',
