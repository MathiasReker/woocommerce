...
class WC_REST_Shipping_Zone_Locations_V2_Controller extends WC_REST_Shipping_Zones_Controller_Base {
	...
	public function register_routes() {
		register_rest_route(
			$this->namespace, '/' . $this->rest_base . '/(?P<id>[\d]+)/locations', [
				'args'   => [
					'id' => [
	...
	public function update_items( $request ) {
		...
		if ( 0 === $zone->get_id() ) {
			return new WP_Error( 'woocommerce_rest_shipping_zone_locations_invalid_zone', __( 'The "locations not covered by your other zones" zone cannot be updated.', 'woocommerce' ), [ 'status' => 403 ] );
		}
		...
		foreach ( (array) $raw_locations as $raw_location ) {
			...
			if ( ! in_array( $type, [ 'postcode', 'state', 'country', 'continent' ], true ) ) {
				continue;
			}
