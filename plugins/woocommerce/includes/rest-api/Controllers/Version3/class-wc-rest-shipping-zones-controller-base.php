...
abstract class WC_REST_Shipping_Zones_Controller_Base extends WC_REST_Controller {
	...
	protected function get_zone( $zone_id ) {
		...
		if ( false === $zone ) {
			return new WP_Error( 'woocommerce_rest_shipping_zone_invalid', __( 'Resource does not exist.', 'woocommerce' ), [ 'status' => 404 ] );
		}
	...
	public function get_items_permissions_check( $request ) {
		if ( ! wc_shipping_enabled() ) {
			return new WP_Error( 'rest_no_route', __( 'Shipping is disabled.', 'woocommerce' ), [ 'status' => 404 ] );
		}
	...
	public function delete_items_permissions_check( $request ) {
		...
		if ( ! wc_rest_check_manager_permissions( 'settings', 'delete' ) ) {
			return new WP_Error( 'woocommerce_rest_cannot_edit', __( 'Sorry, you are not allowed to delete this resource.', 'woocommerce' ), [ 'status' => rest_authorization_required_code() ] );
		}
